...
class Personaje extends Objeto
{
    ...
    public static function guardaInventario($personaje){
        ...
        if(self::eliminaGastados($personaje)){
            return true;
        }

        return false;
    }
    public static function eliminaGastados($personaje){
        if(isset($_SESSION['idPartida'])){
            if(array_key_exists('inventarioConsumidos',$personaje)){

                $app = App::getSingleton();
                $conn = $app->conexionBd();

                $idInventario=self::buscaInventarioPorId();
                $arrayObjetosInvPartida=self::objetosInventarioPartida($idInventario);

                for($z=0;$z<sizeof($personaje['inventarioConsumidos']);$z++){
                    for ($j=0; $j<sizeof($arrayObjetosInvPartida); $j++) { 
                          $idConsumible=$personaje['inventarioConsumidos'][$z];
                          if($arrayObjetosInvPartida[$j]['idConsumible']=$idConsumible){
                               $query = sprintf("DELETE FROM inventariocontiene WHERE inventariocontiene.idInventario =%s AND inventariocontiene.idConsumible = %s",$idInventario,$idConsumible);
    ...
    public static function objetosInventarioPartida($idInventario){
        $result_array=array();
        $app = App::getSingleton();
        $conn = $app->conexionBd();
    ...
    public static function cargaPersonajesIdUs($idUsuario)
    {
        $result_array=array();
        $app = App::getSingleton();
        $conn = $app->conexionBd();
        $query = sprintf("SELECT idObjeto FROM comprados WHERE comprados.tipo='personaje' AND comprados.idUsuario=%s", $idUsuario);
        $rs = $conn->query($query);
        if ($rs) {
            if ($rs->num_rows > 0) {
                while($row = $rs->fetch_assoc()) {
