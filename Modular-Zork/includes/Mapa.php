...
public static function buscaMapaPorNombre($nombre)
{
  ...
  return false;
    ...
    //Muestra la informaciÃ³n del mapa en la tienda
  ...
  public static function getMapaPartida($idPartida){
		
      $app = App::getSingleton(); 
      $conn = $app->conexionBd();
     // $query = sprintf("SELECT partida.idMapa FROM partida WHERE  partida.idPartida=%s",$idPartida);
      $query = sprintf("SELECT mapas.* FROM mapas, partida WHERE partida.idPartida=%s and mapas.id=partida.idMapa", $idPartida);
      $rs = $conn->query($query);	  
      if ($rs && $rs->num_rows == 1) {
		  $fila = $rs->fetch_assoc(); 
		  $mapa = new Mapa($fila['id'], $fila['nombre'], $fila['dificultad'], $fila['precio'], $fila['numMazmorras'],$fila['recompensa'],$fila['propietario'], $fila['rutaImagen'], $fila['descripcion'], $fila['valoracion'], $fila['numJugado'], $fila['terminadoCreado']);
