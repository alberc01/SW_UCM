...
public static function signin($name,$usermail, $password)
{
  if(!self::buscaUsuario($usermail) && !self::buscaUsuarioPorNombre($name)){
    ...
    if ($rs && $user) {
      ...
      $tipo="personaje";
      $idObjeto=2;
      Compra::compraNuevoUsuario($user->id(), $precio, $tipo, $idObjeto);
      return $user;
    }
    else{
      echo"$conn->error";
...
public function actualizarUsuarioFinPartida($mapa)
{
  $app = App::getSingleton();
  $conn = $app->conexionBd();
  $name = $app->nombreUsuario();
  $user = self::buscaUsuarioPorNombre($name);
  if($user){
   $puntos = ($mapa->getDificultad()*100);
   $monedas = 100 + $mapa->getRecompensa();

   $puntos = ($puntos + $user->getPuntos());  
   $monedas = ($monedas + $user->getMonedas());

   $query = sprintf("UPDATE usuarios SET puntos ='$puntos', monedas ='$monedas' WHERE usuarios.id = %s",$user->id());
   $rs = $conn->query($query);
   if($rs){
	// echo"puntos y monedas actualizados";
     return true;
   }
   else{
     echo"$conn->error";
...
public function sumaMonedas($cant){
    $app = App::getSingleton();
    $conn = $app->conexionBd();
    $cant += self::getMonedas();
    $id = self::id();
    $query = sprintf("UPDATE usuarios SET monedas = %s WHERE id = %s" ,$conn->real_escape_string($cant),$conn->real_escape_string($id));
    $rs = $conn->query($query);
    if($rs){
      return true;
    }else
