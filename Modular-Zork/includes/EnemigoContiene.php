...
class EnemigoContiene extends Objeto
{
  ...
  public static function cargaEnemigos($idMazmorra){
   ...
   public static function cargaTodosEnemigos(){
        $consulta = array();
        $enemigo = array();
        $app = App::getSingleton(); 
        $conn = $app->conexionBd();
        
        $query = sprintf("SELECT id,nombre,fuerza,habilidad,vida,precio,rutaImagen, w,h,tipo FROM enemigo");
        $rs = $conn->query($query);
        if ($rs) {
            if ($rs->num_rows > 0) {

                while($row = $rs->fetch_assoc()) {
                    array_push($enemigo, new EnemigoContiene($row['id'],$row['nombre'],$row['fuerza'],$row['habilidad'],$row['vida'],$row['precio'],$row['rutaImagen'],
                        0,0,$row['w'],$row['h'],$row['tipo']));
                    array_push($consulta,$row);
                }
