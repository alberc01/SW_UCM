...
class Compra
{
	...
	public function compraNuevoUsuario($idUser, $precio, $tipo, $idObjeto)
	{
		...
		VALUES(%s,%s,'%s',%s)",$idUser,$idObjeto,$conn->real_escape_string($tipo),$precio);
	...
	$app = App::getSingleton();
    	...
    	if(self::objetoNoComprado($conn)){
    		if(self::monedasSuficientes($conn)){
		    	...
		    	$tipo = self::type();
		    	//Inserta la compra en la tabla comprados
		    	$query = sprintf("INSERT INTO comprados(idUsuario,idObjeto,tipo,precio)VALUES(%s,%s,'%s',%s)",$idUser,$idObjeto,$conn->real_escape_string($tipo),$precio);
		    	$res = $conn->query($query);
				if($res){
				...
				<p id='masMonedas'><a href='aniadirMonedas.php'>Consigue mas monedas aquí</a></p>";
    		}
    	}else{
