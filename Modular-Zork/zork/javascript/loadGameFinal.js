...
$(document).on('click', '#prueba', function() {
	$(function(){ 
        ...
        demo=1;
        cargado=0
        CargaPersonaje(null,1,1,demo,cargado);
...
function choicePartida(myArray,cargado){
    ...
    for(i=0; i<myArray.length; i++){     
        panel.append('<legend><input type="Radio" name="partida" value="'
            + i +'">Partida: '+i+ "<strong> Mapa: </strong>" + myArray[i].nombreMapa + "<strong> Personaje: </strong>" 
            + myArray[i].nombrePj + "</legend><p>inicio:   "+ myArray[i].fechaInicio+"</p> ");
    }
        ...
        success: function (msg){
            ...
            if(result.length !== 0){
                ...
                for (var i = 0; i < myArray[0].length; i++) {
                    ...
                    mapa.personaje.setVidaMax(myArray[0][i].vidaMax);
                    mapa.personaje.setFuerza(myArray[0][i].fuerza);
                    mapa.personaje.setVidaAct(myArray[0][i].vidaAct);
                    mapa.personaje.setHabilidad(myArray[0][i].habilidad);
                    mapa.personaje.setTraspa(myArray[0][i].trasparencia);
                    mapa.personaje.actualizaTraspa();
                }
